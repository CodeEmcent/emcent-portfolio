  ...
  return (
    ...
    >
      ...
      <div>
        ...
        >
          <strong style={{ color: 'var(--text)', fontWeight: 500 }}>MSc MBIT, Management of Buisness Information Technology graduate</strong>
            ...
            {stats.map(({ num, label }) => (
              <div key={label}>
                <p className="font-serif text-[2rem] leading-none" style={{ color: 'var(--text)' }}>
                  {num.replace('+', '')}<span style={{ color: 'var(--accent)' }}>{num.includes('+') ? '+' : ''}</span>
                </p>
